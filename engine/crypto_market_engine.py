...
Uses Delta Exchange India public API.
...
COINS = {
    'L1':     ['SOL','BNB','XRP','ADA','AVAX','DOT','ATOM','APT','SUI','TIA'],
    'DeFi':   ['LINK','UNI','AAVE','MKR','SNX'],
    'L2':     ['MATIC','ARB','OP','STRK'],
    'Infra':  ['INJ','SEI','RUNE','FET'],
...
def fetch_all_products():
    data = safe_get(f"{BASE_URL}/v2/products", {'contract_type': 'perpetual_futures'})
    if not data or not data.get('success'):
        return {}
    products = data.get('result', [])
    product_map = {}
    for p in products:
        symbol = p.get('underlying_asset', {}).get('symbol', '')
        if symbol:
            product_map[symbol] = p
    print(f"   ✅ {len(product_map)} perpetual futures products")
    return product_map
...
def fetch_all_tickers():
    ...
    for t in tickers:
        sym = t.get('symbol', '')
        underlying = sym.replace('USD','').replace('USDT','').replace('PERP','')
        ticker_map[underlying] = t
    print(f"   ✅ {len(ticker_map)} tickers")
    return ticker_map

def fetch_ohlcv(product_symbol, limit=60):
    end_time   = int(datetime.now(timezone.utc).timestamp())
    start_time = end_time - (limit * 86400)
    data = safe_get(f"{BASE_URL}/v2/history/candles", {
        'resolution': '1d', 'symbol': product_symbol,
...
def fetch_funding_rate(product_symbol):
    data = safe_get(f"{BASE_URL}/v2/products/{product_symbol}/funding_rate")
    if not data or not data.get('success'):
        return None
...
def process_coin(coin, ticker_map, product_map):
    ticker  = ticker_map.get(coin)
    product = product_map.get(coin)
    if not ticker:
        return None
    ...
    product_symbol = product.get('symbol') if product else None

    ohlcv = None
    if product_symbol:
        ohlcv = fetch_ohlcv(product_symbol)
        time.sleep(0.2)

    if ohlcv and len(ohlcv[0]) >= 20:
    ...
    funding = None
    if product_symbol:
        funding = fetch_funding_rate(product_symbol)
        time.sleep(0.1)

    return {
        'symbol': f"{coin}USD", 'name': coin,
        'sector': SECTOR_MAP.get(coin, 'Other'),
        'price': round(price, 6), 'change_pct': chg_pct,
        'volume_usd': round(vol, 0),
        'ema9': round(e9,6), 'ema20': round(e20,6), 'ema50': round(e50,6),
        'rsi': r, 'momentum': m, 'trend': tr,
        'volume_signal': vs, 'vol_ratio': round(vr,2),
        'technical_score': ts, 'funding_rate': funding,
        'open_interest': None, 'ls_ratio': None,
    }
...
def run_market_engine():
    ...
    product_map = fetch_all_products()
    time.sleep(0.5)
    ...
    if not ticker_map:
        print("   ⚠ No data from Delta Exchange")
        output = {
    ...
    adv   = sum(1 for t in all_t if float(t.get('change', 0)) > 0)
    dec   = len(all_t) - adv
    ...
    for sector, coins in COINS.items():
        ...
        sc = []
        for coin in coins:
            data = process_coin(coin, ticker_map, product_map)
            if data:
                sc.append(data)
        ...
        sector_results[sector] = {
            'score': ss, 'coin_count': len(sc), 'top_coins': [c['name'] for c in top3],
    ...
    print(f"\n✅ Done — {len(all_coins_data)} coins | BTC ${reference.get('BTC',{}).get('price','N/A')} | {direction}")
    return output
