...
def run_market_engine():
    ...
    for sector in COINS.keys():
        ...
        if not sc:
            continue
        ss   = score_sector(sc)
        top3 = sorted(sc, key=lambda x: x.get('technical_score',0), reverse=True)[:3]
        sector_results[sector] = {
            'score': ss, 'coin_count': len(sc),
            'top_coins': [c['name'] for c in top3],
            'avg_rsi': round(sum(c.get('rsi',50) for c in sc)/len(sc), 1),
            'bullish_count': sum(1 for c in sc if c.get('change_pct',0) > 0),
            'bearish_count': sum(1 for c in sc if c.get('change_pct',0) <= 0),
        }
